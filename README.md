...
### Влияние длины фрагмента (числа кадров)
Re-оценка обученных чекпоинтов при 4/8/16/32 кадрах (`results/frame_sweep.png`,
скрипт [src/frame_sweep.py](src/frame_sweep.py)):

| frames | I3D acc | R(2+1)D acc |
|:---:|:---:|:---:|
| 4  | 0.685 | 0.613 |
| 8  | 0.766 | 0.673 |
| 16 | 0.772 | 0.757 |
...
Для **3D-CNN качество растёт с числом кадров** и выходит на плато к 16-32: коротких
4 кадров не хватает, чтобы уловить динамику, а после 16 прирост невелик (≈+0.01) при
росте задержки. **16 кадров — разумный компромисс** точность/скорость. (TSN исключён
из этого графика: его temporal-pooling голова по построению почти нечувствительна к
длине, а результат свипа разошёлся с held-out eval — артефакт пересборки модели.)

### Анализ ошибок (по лучшей модели, VideoMAE)
Из 346 тестовых роликов — 43 ошибки. Per-class F1 высок почти везде (climb, golf,
pullup, ride_bike, draw_sword, shoot_bow: **0.95-1.00**), но проседают:

| класс | F1 | куда путается |
|---|:---:|---|
| walk | 0.64 | 12× → `run`, 5× → `wave` |
| run  | 0.72 | 8× → `walk`, 3× → `fencing` |
| wave | 0.80 | 3× → `walk` |

**Главный источник ошибок — пара `run`↔`walk`** (20 из 43 промахов): оба класса —
локомоция, отличаются в основном скоростью, которую при 8-16 кадрах легко спутать
(см. 3 ошибочных примера в `reports/examples/` — все `walk`→`run` с умеренной
уверенностью 0.53-0.68, причём верный класс почти всегда второй в top-3). А вот
«мечевая» пара `fencing`/`draw_sword` модель **различает уверенно** (F1 0.90/0.97) —
их жесты визуально расходятся сильнее, чем ходьба и бег.

**3 удачных примера** (`reports/examples/good_*`): golf, pullup, golf — уверенность
0.97, характерная статичная поза + выраженное движение.

**Идеи по улучшению:** (1) полное дообучение backbone (`--no-freeze`) — особенно
поднимет TSN/TSM; (2) больше кадров / выше FPS-сэмплинг, чтобы развести `run`/`walk`
по темпу; (3) взвешивание классов или oversampling сложных; (4) оптический поток как
доп. вход для разделения близких по виду движений.
